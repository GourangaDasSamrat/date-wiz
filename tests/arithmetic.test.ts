...
describe("isBefore() / isAfter()", () => {
  ...
  test("isBefore false when after", () =>
  ...
  test("same day returns true", () => {
    expect(
      isSameDay(
        ...
        new Date("2024-05-15T22:00:00"),
      ),
    ).toBe(true);
  });
  test("different day returns false", () => {
    expect(isSameDay(new Date("2024-05-15"), new Date("2024-05-16"))).toBe(
      false,
    );
  });
});
...
describe("clampDate()", () => {
  ...
  test("clamps below min", () => {
    expect(clampDate(new Date("2023-06-01"), min, max).getFullYear()).toBe(
      2024,
    );
  });
  test("clamps above max", () => {
    expect(clampDate(new Date("2025-06-01"), min, max).getFullYear()).toBe(
      2024,
    );
  });
